...
df <- read.csv("musicdata.csv",fileEncoding = "ISO-8859-1")
df <- df %>% drop_na
nrow(df)
...
server <- function(input, output) {
  ...
  observeEvent(input$submit, {
    ...
    # if no duplicates, proceed with the rest of the code
      ...
      energy <- mean(chosen_songs_data$Energy)
      danceability <- mean(chosen_songs_data$Danceability)
      ...
      recommended_songs <- df %>% filter(Energy >= energy, Danceability >= danceability, Beats_Per_Minute> bpm, Duration>duration, Popularity>popularity)
